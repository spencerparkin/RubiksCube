...
void SolverForCase3::PerformOrangeCrossPositioningStage( const RubiksCube* rubiksCube, RubiksCube::RotationSequence& rotationSequence )
{
	...
	// Here, (unlike the next stage), we don't have to solve the entire problem right away.
	CycleActionList cycleActionList;
	SolveQuadAndTriCycleProblem( order, cycleActionList, false );
	wxASSERT( cycleActionList.size() == 0 || cycleActionList.size() == 1 );

	if( cycleActionList.size() == 1 )
	{
		RubiksCube::RelativeRotationSequence relativeRotationSequence;
		RubiksCube::Perspective perspective;

		CycleAction cycleAction = cycleActionList.front();
...
void SolverForCase3::PerformOrangeCornerPositioningStage( const RubiksCube* rubiksCube, RubiksCube::RotationSequence& rotationSequence )
{
	...
	// Unfortunately, we can't solve this stage, (like the last stage), in sub-stages,
	// (we must solve it completely right now), because intermediate stages,
	// (sub-stages), of this stage may cause the previous stage to become undone.
	// What this all really amounts to is code that is not as elegant as I would have hoped.  :/
	CycleActionList cycleActionList;
	SolveQuadAndTriCycleProblem( order, cycleActionList, true );
	
	RubiksCube::RelativeRotationSequence relativeRotationSequence;
	RubiksCube::Perspective perspective;

	for( CycleActionList::iterator iter = cycleActionList.begin(); iter != cycleActionList.end(); iter++ )
	{
		CycleAction cycleAction = *iter;
		switch( cycleAction.cycleType )
		{
			case CycleAction::QUAD_CYCLE_FORWARD:
			{
			...
			case CycleAction::TRI_CYCLE_FORWARD:
			{
				...
				perspective = standardPerspectivesNegated[ cycleAction.triCycleInvariantIndex ];
				break;
			}
			case CycleAction::TRI_CYCLE_BACKWARD:
			{
		...
		// The translator routine here should append to our list each iteration of the loop.
		if( relativeRotationSequence.size() > 0 )
			rubiksCube->TranslateRotationSequence( perspective, relativeRotationSequence, rotationSequence );
	}
}
...
//==================================================================================================
/*static*/ void SolverForCase3::SolveQuadAndTriCycleProblem( int* order, CycleActionList& cycleActionList, bool solveCompletely /*= false*/ )
{
	cycleActionList.clear();
	...
	while( !( order[0] == 0 && order[1] == 1 && order[2] == 2 && order[3] == 3 ) )
	{
		...
		if( normalizingShift == 4 )
		{
			...
			for( cycleAction.quadCycleCount = 0; cycleAction.quadCycleCount < 4; cycleAction.quadCycleCount++ )
				if( order[ cycleAction.quadCycleCount ] == 0 )
					break;
			wxASSERT( cycleAction.quadCycleCount < 4 && cycleAction.quadCycleCount > 0 );
		}
		else
		{
		...
		cycleActionList.push_back( cycleAction );

		// The caller may not need us to completely solve the stage.
		if( !solveCompletely )
			break;
		...
		switch( cycleAction.cycleType )
		{
			...
			case CycleAction::QUAD_CYCLE_BACKWARD:
			{
				int offset = cycleAction.quadCycleCount;
				if( cycleAction.cycleType == CycleAction::QUAD_CYCLE_FORWARD )
					offset = 4 - offset;
				for( int index = 0; index < 4; index++ )
					newOrder[ index ] = order[ ( index + offset ) % 4 ];
				break;
			}
			case CycleAction::TRI_CYCLE_FORWARD:
			case CycleAction::TRI_CYCLE_BACKWARD:
			{
				int offset = 1;
				if( cycleAction.cycleType == CycleAction::TRI_CYCLE_FORWARD )
					offset = 3;
				for( int index = 0; index < 4; index++ )
				{
					int newIndex = index;
					if( index != cycleAction.triCycleInvariantIndex )
					{
						newIndex = ( index + offset ) % 4;
						if( newIndex == cycleAction.triCycleInvariantIndex )
							newIndex = ( newIndex + offset ) % 4;
					}
					newOrder[ index ] = order[ newIndex ];
				}
				break;
			}
		}
